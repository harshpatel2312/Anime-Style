...
epochs = 5                    # runs epochs: 0 .. epochs-1
lr_G = 2e-4
lr_D = 1e-4
log_interval = 10
ckpt_every = 500              # save every N batches (was 100)
ckpts_to_keep = 3             # rotate older checkpoints
...
criterion_l1  = nn.L1Loss().to(device)  # used as content loss to input
...
# ========= TRAINING ============
try:
    for epoch in range(resume_epoch, epochs):  # run 0..epochs-1
        G.train()
        D.train()
        epoch_loss_G = 0.0
        epoch_loss_D = 0.0
        ...
        for i, (anime_imgs, real_imgs) in enumerate(zip(anime_train_loader, cycle(real_train_loader))):
            ...
            with torch.no_grad():
                fake_anime_detached = G(real_imgs)
            pred_real = D(anime_imgs)
            pred_fake = D(fake_anime_detached)
            ...
            # *** Key change: content-anchored loss to input real image ***
            loss_content = criterion_l1(fake_anime, real_imgs) * 5.0  # weight is tunable

            loss_G = loss_G_GAN + loss_content
            loss_G.backward()
            optimizer_G.step()
            ...
            if (i + 1) % log_interval == 0:
                print(f"[Epoch {epoch}/{epochs-1}] [Batch {i + 1}] "
            ...
            if (i + 1) % ckpt_every == 0:
                ...
                folders = sorted(os.listdir(checkpoint_dir))
                if len(folders) > ckpts_to_keep:
                    for old in folders[:-ckpts_to_keep]:
